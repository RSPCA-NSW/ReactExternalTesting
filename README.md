...
└── uiBundles/
    └── ReactExternalTesting/         # React UI Bundle (source, config, tests)
```
...
This installs the UI Bundle dependencies, builds the app, and opens the dev server at http://localhost:5173. For manual build and test instructions, see the [UI Bundle README](force-app/main/default/uiBundles/ReactExternalTesting/README.md).
...
```bash
npx @salesforce/ui-bundle-features-experimental install <feature-name> --ui-bundle-dir ReactExternalTesting
```
...
```bash
cd force-app/main/default/uiBundles/ReactExternalTesting && npm install && npm run build && cd -
